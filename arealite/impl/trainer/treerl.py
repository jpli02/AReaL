...
logger = logging.getLogger("TreeRL Trainer", "system")

class TreeRLTrainer(Trainer):
    def __init__(
        self,
        args: TrainingArgs,
        ...
        if self.rollout_controller is None:
                raise ValueError("TreeRL Trainer requires a rollout controller.")

        assert trainer_config.treerl is not None
        self.config: TreeRLTrainerConfig = trainer_config.treerl
        ...
        for epoch in range(total_epochs):
            for step, data in enumerate(self.train_dataloader):
                ...
                if dist.get_rank() == 0:
                    ...
                    for i, stats in enumerate(mb_stats):
                        logger.info(
                            f"TreeRL training stats ({i + 1}/{len(mb_stats)}):\n{tabulate_stats(stats)}"
                        )
